...
deploy_target_branch="${OBMONDO_DEPLOY_TARGET_BRANCH:-master}"
...
find "${config_repo_path}" -mindepth 1 -maxdepth 1 -type d -not -name .\* | while read -r cluster_dir; do
    cluster="$(basename "${cluster_dir}")"
    # cluster_jsonnet="${cluster_dir}/${cluster}-vars.jsonnet"
    ./build/kube-prometheus/build.sh "$cluster_dir"

    git -C "${config_repo_path}" add -f "${cluster}/kube-prometheus"
done
