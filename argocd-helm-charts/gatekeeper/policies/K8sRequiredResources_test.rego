...
parameters := {"limits": ["memory"], "requests": ["cpu", "memory"]}
test_without_resources_violations {
  input := {"review": review([ctr_without_resources("test")]), "parameters": parameters}
...
test_requests_cpu_violations {
  input := {"review": review([ctr_requests_cpu("test", 1)]), "parameters": parameters}
  results := violation with input as input
  trace(sprintf("results - <%v>", [results]))
  count(results) == 2
}
...
test_requests_memory_violations {
  input := {"review": review([ctr_requests_memory("test", 1)]), "parameters": parameters}
  results := violation with input as input
  trace(sprintf("results - <%v>", [results]))
  count(results) == 2
}
...
test_requests_violations {
  input := {"review": review([ctr_requests("test", 1)]), "parameters": parameters}
  results := violation with input as input
  trace(sprintf("results - <%v>", [results]))
  count(results) == 1
}
...
test_limits_cpu_violations {
  input := {"review": review([ctr_limits_cpu("test", 1)]), "parameters": parameters}
  results := violation with input as input
  trace(sprintf("results - <%v>", [results]))
  count(results) == 2
}
...
test_limits_memory_violations {
  input := {"review": review([ctr_limits_memory("test", 1)]), "parameters": parameters}
  results := violation with input as input
  trace(sprintf("results - <%v>", [results]))
  count(results) == 1
}
...
test_limits_violations {
  input := {"review": review([ctr_limits("test", 1)]), "parameters": parameters}
  results := violation with input as input
  trace(sprintf("results - <%v>", [results]))
  count(results) == 1
}
...
test_with_resources_no_violations {
  input := {"review": review([ctr_with_resources("test", 1)]), "parameters": parameters}
  results := violation with input as input
  trace(sprintf("results - <%v>", [results]))
  count(results) == 0
}

# multiple containers, "parameters": {"limits": ["memory"], "requests": ["cpu", "memory"]}
test_multiple_without_resources_violations {
  input := {"review": review([ctr_without_resources("test1"),ctr_without_resources("test2"),ctr_without_resources("test3")]), "parameters": parameters}
  results := violation with input as input
  trace(sprintf("results - <%v>", [results]))
  count(results) == 6
}
...
test_multiple_with_resources_no_violations {
  input := {"review": review([ctr_with_resources("test1", 1),ctr_with_resources("test2", 2),ctr_with_resources("test3", 3)]), "parameters": parameters}
  results := violation with input as input
  trace(sprintf("results - <%v>", [results]))
  count(results) == 0
}
